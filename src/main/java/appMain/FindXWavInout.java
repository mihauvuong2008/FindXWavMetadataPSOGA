...
//		return 2 * Math.pow(x, 5) + 3 * Math.pow(x, 4) - 8 * Math.pow(x, 3) + 2 * Math.pow(x, 2) + 1;
		double function = x * Math.sin(Math.sin(x) * x + x * Math.sin(x + Math.sin(x) * x)) * Math.sin(x)
				+ x * Math.cos(Math.cos(x) * x + x * Math.cos(x + Math.cos(x) * x)) * Math.cos(x);

		return x * Math.sin(Math.sin(x) * x + x * Math.sin(x + Math.sin(x) * x)) * Math.sin(x)
				+ x * Math.cos(Math.cos(x) * x + x * Math.cos(x + Math.cos(x) * x)) * Math.cos(x);
	}
...
//		System.out.println("rs: " + rs + " y " + y);
	...
	public static double getTotalError(ArrayList<Double> metadata, double valueLevel, double y) {
